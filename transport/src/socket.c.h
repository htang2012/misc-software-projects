...
#if 0 
static int findInterfacesenv(const char* prefixList, char* names, union socketAddress *addrs, int sock_family, int maxIfNameSize, int maxIfs) {
#ifdef ENABLE_TRACE
  char line[1024];
#endif
  struct netIf userIfs[MAX_IFS];
  unsigned int searchNot = prefixList && prefixList[0] == '^';
  if (searchNot) prefixList++;
   searchExact = prefixList && prefixList[0] == '=';
...
static  matchSubnet(struct ifaddrs local_if, union socketAddress* remote) {
  /* Check family first */
...
static ncclResult_t GetSocketAddrFromString(union socketAddress* ua, const char* ip_port_pair) {
  if (!(ip_port_pair && strlen(ip_port_pair) > 1)) {
   ...
   unsigned int ipv6 = ip_port_pair[0] == '[';
  ...
  if (!ipv6) {
    ...
    if (i == len) {
     ...
     global_scope = (j == -1 ? true : false);     // If no % found, global scope; otherwise, link scope
...
static int findInterfaces(char* ifNames, union socketAddress *ifAddrs, int ifNameMaxSize, int maxIfs) {
  ...
  ////nIfs = findInterfacesenv(env, ifNames, ifAddrs, sock_family, ifNameMaxSize, maxIfs);
  return nIfs;
}
