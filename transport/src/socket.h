...
#define SYSCHECKVAL(call, name, retval) do { \
        ...
        if (retval == -1) { \
                NCCL_OFI_WARN("Call to " name " failed : %s", strerror(errno)); \
...
#define SYSCHECKSYNC(call, name, retval) do { \
          ...
          if (retval == -1 && (errno == EINTR || errno == EWOULDBLOCK || errno == EAGAIN)) { \
                      NCCL_OFI_INFO(NCCL_ALL,"Call to " name " returned %s, retrying", strerror(errno)); \
                    } else { \
                                break; \
                              } \
} while(true)
...
#define NCCLCHECK(call) do { \
        ...
        if (res != ncclSuccess) { \
                ...
                NCCL_OFI_INFO(NCCL_ALL,"%s:%d -> %d", __FILE__, __LINE__, res);    \
                return res; \
        } \
} while (0)
...
static int findInterfacesv(const char* prefixList, char* names, union socketAddress *addrs, int sock_family, int maxIfNameSize, int maxIfs) {
  char line[1024];
  ...
  for (interface = interfaces; interface && found < maxIfs; interface = interface->ifa_next) {
    ...
    NCCL_OFI_TRACE(NCCL_INIT|NCCL_NET,"Found interface %s:%s", interface->ifa_name, socketToString(interface->ifa_addr, line));
...
static bool matchSubnet(struct ifaddrs local_if, union socketAddress* remote) {
  ...
  if (family == AF_INET) {
    ...
    remote_subnet.s_addr = remote->sin.sin_addr.s_addr & mask->sin_addr.s_addr;
    ...
    for (int c = 0; c < len; c++) {  //Network byte order is big-endian
      char c1 = local_addr->sin6_addr.s6_addr[c] & mask->sin6_addr.s6_addr[c];
      char c2 = remote->sin6.sin6_addr.s6_addr[c] & mask->sin6_addr.s6_addr[c];
      if (c1 ^ c2) {
        same = false;
        break;
      }
    ...
    same &= (local_addr->sin6_scope_id == remote->sin6.sin6_scope_id);
    return same;
  } else {
    NCCL_OFI_WARN("Net : Unsupported address family type");
    return false;
  }
}
...
static int findInterfaceMatchSubnet(char* ifNames, union socketAddress* localAddrs, union socketAddress* remoteAddr, int ifNameMaxSize, int maxIfs) {
  ...
  for (interface = interfaces; interface && !found; interface = interface->ifa_next) {
    ...
    NCCL_OFI_TRACE(NCCL_INIT|NCCL_NET,"NET : Found interface %s:%s in the same subnet as remote address %s", interface->ifa_name, socketToString(&(localAddrs[found].sa), line), socketToString(&(remoteAddr->sa), line_a));
    found++;
    if (found == maxIfs) break;
  }

  if (found == 0) {
    NCCL_OFI_WARN("Net : No interface found in the same subnet as remote address %s", socketToString(&(remoteAddr->sa), line_a));
  }
  freeifaddrs(interfaces);
  return found;
}
...
static ncclResult_t GetSocketAddrFromString(union socketAddress* ua, const char* ip_port_pair) {
  if (!(ip_port_pair && strlen(ip_port_pair) > 1)) {
    NCCL_OFI_WARN("Net : string is null");
  ...
  if (!ipv6) {
    ...
    if (parseStringList(ip_port_pair, &ni, 1) != 1) {
      NCCL_OFI_WARN("Net : No valid <IPv4_or_hostname>:<port> pair found");
      return ncclInvalidArgument;
    }
    ...
    if ( (rv = getaddrinfo(ni.prefix, NULL, &hints, &p)) != 0) {
      NCCL_OFI_WARN("Net : error encountered when getting address info : %s", gai_strerror(rv));
      return ncclInvalidArgument;
    }
   ...
   //   struct sockaddr_in& sin = ua->sin;
      memcpy(&ua->sin, p->ai_addr, sizeof(struct sockaddr_in));
      ua->sin.sin_family = AF_INET;                        // IPv4
      //inet_pton(AF_INET, ni.prefix, &(sin.sin_addr));  // IP address
      ua->sin.sin_port = htons(ni.port);                   // port
    } else if (p->ai_family == AF_INET6) {
      //struct sockaddr_in6& sin6 = ua->sin6;
      memcpy(&ua->sin6, p->ai_addr, sizeof(struct sockaddr_in6));
      ua->sin6.sin6_family = AF_INET6;                     // IPv6
      ua->sin6.sin6_port = htons(ni.port);                 // port
      ua->sin6.sin6_flowinfo = 0;                          // needed by IPv6, but possibly obsolete
      ua->sin6.sin6_scope_id = 0;                          // should be global scope, set to 0
    } else {
      NCCL_OFI_WARN("Net : unsupported IP family");
      return ncclInvalidArgument;
    }
    ...
    if (i == len) {
      NCCL_OFI_WARN("Net : No valid [IPv6]:port pair found");
      return ncclInvalidArgument;
    }
    ...
    ua->sin6.sin6_flowinfo = 0;                            // needed by IPv6, but possibly obsolete
    ua->sin6.sin6_scope_id = global_scope ? 0 : if_nametoindex(if_name);  // 0 if global scope; intf index if link scope
  }
  return ncclSuccess;
}
...
static int findInterfaces(char* ifNames, union socketAddress *ifAddrs, int ifNameMaxSize, int maxIfs) {
  ...
  if (env && strlen(env) > 1) {
    ...
    nIfs = findInterfacesv(env, ifNames, ifAddrs, sock_family, ifNameMaxSize, maxIfs);
  } else {
    // Try to automatically pick the right one
    // Start with IB
    nIfs = findInterfacesv("ib", ifNames, ifAddrs, sock_family, ifNameMaxSize, maxIfs);
    ...
    if (nIfs == 0) nIfs = findInterfacesv("^docker,lo", ifNames, ifAddrs, sock_family, ifNameMaxSize, maxIfs);
    // Finally look for docker, then lo.
    if (nIfs == 0) nIfs = findInterfacesv("docker", ifNames, ifAddrs, sock_family, ifNameMaxSize, maxIfs);
    if (nIfs == 0) nIfs = findInterfacesv("lo", ifNames, ifAddrs, sock_family, ifNameMaxSize, maxIfs);
  }
  return nIfs;
}
...
static ncclResult_t createListenSocket(int *fd, union socketAddress *localAddr) {
  ...
  if (sockfd == -1) {
    NCCL_OFI_WARN("Net : Socket creation failed : %s", strerror(errno));
    return ncclSystemError;
  }
...
#ifdef ENABLE_NCCL_OFI_TRACE
  char line[1024];
  NCCL_OFI_TRACE(NCCL_INIT|NCCL_NET,"Listening on socket %s", socketToString(&localAddr->sa, line));
#endif
...
static ncclResult_t connectAddress(int* fd, union socketAddress* remoteAddr) {
  ...
  *fd = socket(family, SOCK_STREAM, 0);
  ...
  char line[1024];
  NCCL_OFI_TRACE(NCCL_INIT|NCCL_NET,"Connecting to socket %s", socketToString(&remoteAddr->sa, line));
...
retry:
  ...
  if ((errno == ECONNREFUSED || errno == ETIMEDOUT)) {
    if ((errno == ECONNREFUSED && ++refused_retries < RETRY_REFUSED_TIMES) ||
      ...
      if (refused_retries % 1000 == 0) NCCL_OFI_INFO(NCCL_ALL,"Call to connect returned %s, retrying", strerror(errno));
      usleep(SLEEP_INT);
      goto retry;
    }
  }
  NCCL_OFI_WARN("Connect to %s failed : %s", socketToString(&remoteAddr->sa, line), strerror(errno));
  return ncclSystemError;
}
...
static ncclResult_t socketProgressOpt(int op, int fd, void* ptr, int size, int* offset, int block) {
  ...
  do {
    ...
    if (op == NCCL_SOCKET_RECV && bytes == 0) {
      NCCL_OFI_WARN("Net : Connection closed by remote peer");
      return ncclSystemError;
    }
    if (bytes == -1) {
      if (errno != EINTR && errno != EWOULDBLOCK && errno != EAGAIN) {
        NCCL_OFI_WARN("Call to recv failed : %s", strerror(errno));
        return ncclSystemError;
      } else {
        bytes = 0;
      }
