...
static void setEnvFile(const char* fileName) {
  ...
  while ((read = getline(&line, &n, file)) != -1) {
    ...
    strncpy(envVar, line, s>1024 ? 1024: s);
    envVar[s] = '\0';
    s++;
...
int64_t ncclParam##name() { \
  ...
  if (value == -1LL) { \
    ...
    if (str && strlen(str) > 0) { \
      ...
      if (errno) { \
        NCCL_OFI_INFO(NCCL_ALL,"Invalid value %s for %s, using default %lu.", str, "NCCL_" env, value); \
      } else { \
        value = v; \
        NCCL_OFI_INFO(NCCL_ALL,"%s set by environment to %lu.", "NCCL_" env, value);  \
      } \
    } \
  } \
